...
LocID = 4                                                                                   # [0 = Random, 1 = Random X,Y, Evenly Spaced in Z, 2 = Evenly Spaced + Clusters, 3 = Hexagonal Shape, 4 = Cylindrical Shape]
...
NuCountCut = 6                                                                              # Amount of Neutrino Hits in Event to be counted as correctly identified as neutrino Event

# Scanning Angles
AzimuthVals = [0, 60] #, 60, 120, 180, 240, 300, 360]                                                                           # np.arange(0, 360, 10)
ZenithVals = [0, 10]  #, 10, 20, 30, 40, 50, 60, 70, 80, 90]                                                                            # np.linspace(0, 90, 10)
...
Average_Hits_Per_Run_Event = np.zeros([len(AzimuthVals), len(ZenithVals), runs])
Average_Hits_Event = np.zeros([len(AzimuthVals), len(ZenithVals)])
Time_Store = np.zeros([len(Noise_Rate_Init), len(Nmini)]) 
Correct_Percentage = np.zeros([len(Noise_Rate_Init), len(Nmini)]) 



# Initialize lists and store the results for the anglescan
events_detected = np.zeros([len(AzimuthVals), len(ZenithVals)])
NuCount_values = np.zeros([len(AzimuthVals), len(ZenithVals), runs])
HitsCount_values = np.zeros([len(AzimuthVals), len(ZenithVals), runs])
Correct_Count_values = np.zeros([len(AzimuthVals), len(ZenithVals)])
...
for i in range(len(Noise_Rate_Init)):                                                      # First Loop over amount of Hydrophones
    ...
    for p in range(len(Nmini)):                                                            # Loop over amount of False Hits
        ...
        for run in range(runs):                                                            # Loop over amount of timeslices
            ...
            for a in range(0, len(AzimuthVals)):
                Azimuth = AzimuthVals[a]
                print(f"Azimuth: {Azimuth}")
                for b in range(0, len(ZenithVals)):
                    Correct_Count = 0
                    Event_Log_Done = []
                    Event_Log_Final = []
                    Zenith = ZenithVals[b]
                    ...
                    # Make some seperate Duplicate Hit remover function or something, or is it really needed for non neutrino events? Do I really care about the length of those False Events?
                        # NuCount, HitsCount, Correct_Count, Event_Log_Buffer = HNeutrinoCounter.HNeutrinoCounter(NuPulse, NuCountCut, Correct_Count, Event_Log_Buffer, Azimuth, Zenith)
                        # This Buffer_Keep is only generated if a Neutrino Pulse is generated           
                        # But this is logical as for the noise events I don't really care about the purity
                        # Or anything, as I don't want any noise events in general.                                           
                    ...
                    if len(Event_Log_Buffer) != 0:
                        ...
                        Average_Hits_Per_Run_Event[a,b,run] = sum(Hits_Per_Event)/len(Event_Log_Buffer)
                    ...
                    Correct_Count_values[a, b] = Correct_Count
                    events_detected[a, b] = len(Event_Log_Final)
                    Average_Hits_Event[a, b] = sum(Average_Hits_Per_Run_Event[a,b])/runs

            
            Name = 'Event_NMin' + str(Nmin) + 'Run' + str(NumRun) + 'SubRun' + str(run+1) + 'False' 
            #HEventToFile.HEventToFile(LocEvent + Name, Event_Log_Final, Sources_Storage)
        
        et = time.time()
        time_elapsed = et - st
        Time_Store[i,p] = time_elapsed/runs
    print('Elapsed time ' + str(time_elapsed)) 
        # print('Event Clique Time ' + str(Time_Event_T))
        # print('Event Merge Time ' + str(Time_Merge_T))
...
print(Correct_Count_values)
print(events_detected)

# Now we change the Correct_Count_values to make an average over runs and make the matrix 2D
Correct_Percentage_average = Correct_Count_values
events_detected_average = (events_detected) / Hitsrange[1]

# Check if the original arrays are empty or contain only zeros
print(np.sum(Correct_Percentage_average))
print(Correct_Percentage_average)
print(Correct_Percentage_average.shape)
print(np.sum(events_detected_average))
print(events_detected_average)
print(events_detected_average.shape)


# Write Relevant Data to .txt File #
HData.HDataWriterAngle(NuPulse, OutName, Noise_Rate_Init, Nmini, LocEff, events_detected_average, Correct_Percentage_average, AzimuthVals, ZenithVals)                          # Average_Events_Detected_Hz)