...
Nmini = [20]                                                                                # Nmin value for Clique Algorithm
...
for i in range(len(Noise_Rate_Init)):                                                      # First Loop over amount of Hydrophones
    ...
    for p in range(len(Nmini)):                                                            # Loop over amount of False Hits
        ...
        for run in range(runs):                                                            # Loop over amount of timeslices
            ...
            # The PanDepth and PanSearchDepth are dependent on the distance from the source to the detector
            # If the opening angle is 5 degrees, the pancake depth can be calculated at distance x from the source
            dis = math.sqrt(Source_X**2 + Source_Y**2 + Source_Z**2)
            PanDepth = dis * np.tan(5/180 * np.pi)                                           # Depth of Pancake in m
            PanSearchDepth = dis * np.tan(5/180 * np.pi)                                     # Depth of Pancake in m for HEvent Algorithm
