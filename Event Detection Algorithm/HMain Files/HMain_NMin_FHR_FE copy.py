...
for i in range(len(Noise_Rate_Init)):                                                      # First Loop over amount of Hydrophones
    ...
    for p in range(len(Nmini)):                                                            # Loop over amount of False Hits
        ...
        for run in range(runs):                                                            # Loop over amount of timeslices
            ...
            Event_Log_Done = []
            Event_Log_Final = []
            # Problem! Locations of Hits do not get modified! So rotation is doing nothing in effect! Fix this shit!
            # See is Sympy method works with rotating back, as np rotation causes errors which propagate and will fuck everything up! -- Fixed!
            ...
            for Azimuth in AzimuthVals:
                ...
                for Zenith in ZenithVals:
                    ...
                    # HRotateZenith(Hydrophones, -Zenith)  # When rotating back, reverse order!
                    # HRotateAzimuth(Hydrophones, -Azimuth)
            ...
            #HEventToFile.HEventToFile(LocEvent + Name, Event_Log_Final, Sources_Storage)
                ###### Here Insert Event to File Output Script: Inside the if statement!! ######
                ###### Needs to  be a for loop over range(len(Event_Log_Final)), which then outputs Hit data to file ######
                #
                #
                #
                #
...
for i in range(len(Event_Log_Final)):
    print('Azimuth Equals ' + str(Event_Log_Final[i].Azimuth))
    print('Zenith Equals ' + str(Event_Log_Final[i].Zenith))
    print('Total Hits Equals ' + str(len(Event_Log_Final[i].Hits)))
