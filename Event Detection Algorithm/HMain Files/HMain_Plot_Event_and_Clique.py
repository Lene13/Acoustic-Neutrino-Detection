...
for i in range(len(Noise_Rate_Init)):                                                      # First Loop over amount of Hydrophones
    ...
    # Hits after clique algorythm
    Clique_Hits = Event_Log_Final[0].Hits

    # Rotate all back
    #Hits = HToolbox.HRotateAllBackHits(Hits, np.radians(Sources[0].Azimuth), np.radians(Sources[0].Zenith))
    #Clique_Hits = HToolbox.HRotateAllBackHits(Clique_Hits, np.radians(Sources[0].Azimuth), np.radians(Sources[0].Zenith))



HData.HDataWriterEventPlotClique(OutName, LocEff, Hits, Clique_Hits, AzimuthVals, ZenithVals, Source_X, Source_Y, Source_Z, Hydrophones)               
