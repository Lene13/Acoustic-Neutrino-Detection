...
@numba.jit(nopython=True)
def HClique2(Hits, LocX, LocY, LocZ, LocH, Amp, Type, Nmin, Vsound, TravelSearchDist, PanDepth):
    Count = np.ones(len(Hits))   # Assume match with self
    # Look for Causal Hits & add to Counter #
    for i in range(len(Hits)-1):
        for j in range(i+1, len(Hits)):
    ...
    while p>0:
        ...
        if Count[j] == n:     # number of associated hits is equal to the number of (remaining) hits
            return Hits, Count, LocX, LocY, LocZ, LocH, Amp, Type
...
def HEvent2(Hits, LocX, LocY, LocZ, LocH, Amp, Type, Nmin, PanDepth, Vsound, Max_Dist_Match, TravelSearchDist, pan1, Buffer, BufferX, BufferY, BufferZ, BufferH, BufferA, BufferT):
    # This function checks whether a hit has enough correlated hits within its pancake,
    # if this is the case, the Clique algorithm is employed, then if enough correlated
    # hits remain an event is created in the Event_Log with these hits.
    Event_Write = False
    ...
    if len(Buffer) >= Nmin+1:
        ...
        Buffer, BufferC, BufferX, BufferY, BufferZ, BufferH, BufferA, BufferT = HClique2(Buffer, BufferX, BufferY, BufferZ, BufferH, BufferA, BufferT, Nmin, Vsound, TravelSearchDist, PanDepth)
        if len(Buffer) >= Nmin:
            Event_Write = True

    return Buffer, BufferX, BufferY, BufferZ, BufferH, BufferA, BufferT, Event_Write
...
def HNumbaEvent2(Event_Log, Hits, Hydrophones, HitTimes, LocX, LocY, LocZ, Hydrophone, Amp, Type, Nmin, PanSearchDepth, Vsound, Max_Dist_Match, TravelSearchDist):
    Len_Buffer = []
    for inumba in range(len(Hits)-1):
        Buffer = np.asarray([HitTimes[inumba],0])
        BufferX = np.asarray([LocX[inumba],0])
        ...
        BufferA = np.asarray([Amp[inumba],0])
        BufferT = np.asarray([Type[inumba],0])
        Buffer, BufferX, BufferY, BufferZ, BufferH, BufferA, BufferT, Event_Write = HEvent2(HitTimes, LocX, LocY, LocZ, Hydrophone, Amp, Type, Nmin, PanSearchDepth, Vsound, Max_Dist_Match, TravelSearchDist, inumba, Buffer, BufferX, BufferY, BufferZ, BufferH, BufferA, BufferT)
        Len_Buffer.append(len(Buffer))                                                     
        if Event_Write == True:
            "Write to Event"
            HitsEv = []
            for iEvent in range(len(Buffer)):
                HitsEv.append(HClasses.Hit(HClasses.Hydrophone(BufferH[iEvent], Hydrophones[BufferH[iEvent]-1].IDLoc, BufferX[iEvent], BufferY[iEvent], BufferZ[iEvent]), Buffer[iEvent], BufferA[iEvent], BufferT[iEvent]))
            Event_Log.append(HClasses.Event(HitsEv))

    return Event_Log, Len_Buffer
