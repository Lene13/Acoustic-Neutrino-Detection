...
sys.path.append("/project/antares/lrootsel/effective_volume")
...
zrange = [0,1000]                                                                           # ZLoc range in meters
...
Source_X_Range = [-10000, 10000]
Source_Y_Range = [-10000, 10000]
Source_Z_Range = [0, 3000]
...
# Adapt travel distance to the energy of the neutrino taking into account refraction
# Depending on the hydrophone performance, it's detection threshold changes!!!!!!
Threshold_Amplitude = 5                                                                     # Threshold Amplitude in mPa for a hit to be counted as a neutrino hit

# Assume signal strength diminishes with distance by 1/r^2
TravelDist = (Source_Amp / Threshold_Amplitude)**(1/2)                                      # Maximum detectable Distance from Source
...
NuCountCut = 4                                                                              # Amount of Neutrino Hits in Event to be counted as correctly identified as neutrino Event

# Scanning Angles
AzimuthVals = np.random.uniform(AzimuthRange[0], AzimuthRange[1])                           # np.linspace(0, 360, 10)
ZenithVals = np.random.uniform(ZenithRange[0], ZenithRange[1])                              # np.linspace(0, 90, 10)
...
LocEff = "/project/antares/lrootsel/effective_volume"
#LocEvent = "/data/antares/users/koers/output/Events/13x13x10/Events_FHR/"
OutName = "LocID=" + str(LocID)
...
for i in range(0, len(Locs)):
    # Determine the distance between the source and each of the hydrophones
    Distance = np.sqrt((Source_Loc[0] - Locs[i][0])**2 + (Source_Loc[1] - Locs[i][1])**2 + (Source_Loc[2] - Locs[i][2])**2)
    Dist.append(Distance)

    # Calculate the angle between the source vertex (AzimuthVals, ZenithVals) and each of the hydrophones
    angle = HToolbox.HCalculateAngles(Source_Loc, [AzimuthVals, ZenithVals], Locs[i])
    print(angle)
