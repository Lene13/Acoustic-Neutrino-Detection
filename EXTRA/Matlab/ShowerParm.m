# this script defines a collection of functions that parameterize cosmic ray showers
function tsmc=ShowerParm(r,z,Eo,type)
   % shower parameterisation function
   % currently supports the following type's of shower parametrisations:
		...
		% Inputs:
                    % r radial bin centres
                    % z longitudinal bin centres
                    % Eo Energy (between 10^5 and 10^12 GeV)
  
% change the string in a function handle so that the function picks the right next step
F=str2func(type);

% the following line invokes the function specified by 'type'
tsmc=F(r,z,Eo);
...
tsmc=s.Sm(:,:,round(2*log10(Eo)-9));          % extract specific data from the loaded data based on the energy E0 
tsmc=tsmc';                                   % assign variable tsmc to the data
